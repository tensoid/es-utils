 ...
 */
export function computeMcClusky(
  inputNames: Array<string>,
  minterms: Set<number>,
  dontCares: Set<number>
): string {
  if (minterms.size == 0) return "0";
  if (minterms.size + dontCares.size == Math.pow(2, inputNames.length))
    return "1";
    ...
    mintermTables[iteration - 1].forEach((row, i) => {
      mintermTables[iteration - 1].slice(i + 1).forEach((otherRow) => {
        ...
        mintermTables[iteration].push({
          ...
          bits: row.bits.map((bit, bitIndex) =>
            bitIndex == bitDifferences[0] ? "-" : bit
          ),
          combined: false,
        });
  ...
  let essentialPrimeImplicants = primeImplicants.filter((pi) => {
    ...
    return (
      [...pi.minterms].find((mt) => {
        ...
        return (
          primeImplicants
            .flatMap((pi2) => [...pi2.minterms])
            .filter((mt2) => mt2 == mt).length == 1
        );
      }) != undefined
    );
  });

  let notCoveredMinterms: Set<number> = new Set(
    [...minterms].filter((mt) => {
      return !essentialPrimeImplicants
        .flatMap((pi) => [...pi.minterms])
        .includes(mt);
    })
  );

  if (notCoveredMinterms.size > 0) {
    let primeImplicantsPowerSet = getPowerSet(new Set(primeImplicants)).map(
      (set) => [...set]
    );
    let coveringPrimeImplicantsPowerSet = primeImplicantsPowerSet.filter(
      (pis) => {
        let coveredMinterms = pis.flatMap((pi) => [...pi.minterms]);
        return [...notCoveredMinterms].every((minterm) =>
          coveredMinterms.includes(minterm)
        );
      }
    );

    coveringPrimeImplicantsPowerSet.sort((a, b) => b.length - a.length);
    coveringPrimeImplicantsPowerSet.sort(
      (a, b) =>
        a.flatMap((m) => m.minterms).length -
        b.flatMap((m) => m.minterms).length
    );
...
function getPowerSet<T>(set: Set<T>): Array<Set<T>> {
  return [...set]
    .reduce(
      (subsets, value) =>
        subsets.concat(subsets.map((subset) => [value, ...subset])),
      [[]] as Array<Array<T>>
    )
    .map((arr) => new Set(arr));
}
